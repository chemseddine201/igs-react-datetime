...
export default class Datetime extends React.Component {
	static propTypes = {
		...
		nextIcon: TYPES.oneOfType([TYPES.string, TYPES.element]),
		prevIcon: TYPES.oneOfType([TYPES.string, TYPES.element]),
		minDate: datetype,
		maxDate: datetype,
	}
	...
	_renderCalendar = () => {
		...
		const viewDate = state.viewDate.clone();

		let viewProps = {
			viewDate: viewDate,
			selectedDate: this.getSelectedDate(),
			isValidDate: () => {
				if (props.isValidDate instanceof Function) {
					return props.isValidDate(viewDate);
				} else if (state.minDate && state.maxDate) {
					return (viewDate).isBetween(state.minDate, state.maxDate);
				} else if (state.minDate) {
					return (viewDate).isAfter(state.minDate);
				} else if (state.maxDate) {
					return (viewDate).isBefore(state.maxDate);
				}
				return true;
			},
		...
		switch ( state.currentView ) {
			case viewModes.YEARS:
				// Used viewProps
				// { viewDate, selectedDate, renderYear, isValidDate, navigate, showView, updateDate, prevIcon, nextIcon, minDate, maxDate }
				viewProps.renderYear = props.renderYear;
				return <YearsView {...viewProps } />;
			
			case viewModes.MONTHS:
				// { viewDate, selectedDate, renderMonth, isValidDate, navigate, showView, updateDate, prevIcon, nextIcon, minDate, maxDate }
				viewProps.renderMonth = props.renderMonth;
				return <MonthsView {...viewProps} />;
			
			case viewModes.DAYS:
				// { viewDate, selectedDate, renderDay, isValidDate, navigate, showView, updateDate, timeFormat , prevIcon, nextIcon, minDate, maxDate }
	...
	getInitialState() {
		...
		let selectedMinDate = this.parseDate( props.minDate || props.initialValue, inputFormat );
		let selectedMaxDate = this.parseDate( props.maxDate || props.initialValue, inputFormat );
		console.log(selectedMinDate, selectedMaxDate);
		this.checkTZ();
		...
		return {
			...
			minDate: selectedMinDate && selectedMinDate.isValid() ? selectedMinDate : undefined,
			maxDate: selectedMaxDate && selectedMaxDate.isValid() ? selectedMaxDate : undefined
		};
	}
