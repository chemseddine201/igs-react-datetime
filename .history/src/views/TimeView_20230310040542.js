	...
	renderCounter( type, value ) {
		...
		return (
			<div key={ type } className="rdtCounter">
				<span className="chevron top rdtBtn" onMouseDown={ e => this.onStartClicking( e, 'increase', type)}></span>
				<div className="rdtCount">{ value }</div>
				<span className="chevron bottom rdtBtn" onMouseDown={ e => this.onStartClicking( e, 'decrease', type)}></span>
			</div>
		);
	}
