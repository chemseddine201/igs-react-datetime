	...
	renderCounter( type, value ) {
		...
		return (
			<div key={ type } className="rdtCounter">
				<span className="" onMouseDown={ e => this.onStartClicking( e, 'increase', type)}><span className="chevron top"></span></span>
				<div className="rdtCount">{ value }</div>
				<span className="" onMouseDown={ e => this.onStartClicking( e, 'decrease', type)}><span className="chevron bottom"></span></span>
			</div>
		);
	}
