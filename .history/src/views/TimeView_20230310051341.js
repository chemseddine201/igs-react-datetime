...
export default class TimeView extends React.Component {
	...
	renderCounter( type, value ) {
		...
		return (
			<div key={ type } className="rdtCounter">
				<span className="chevron top" onMouseDown={ e => this.onStartClicking( e, 'increase', type)}></span>
				<div className="rdtCount">{ (type === 'hours' && value < 10) ? `0${value}` : value }</div>
				<span className="chevron bottom" onMouseDown={ e => this.onStartClicking( e, 'decrease', type)}></span>
			</div>
		);
	}
	...
	renderHeader() {
		...
		return (
			<thead>
				<tr>
					<td className="rdtSwitch" colSpan="3" onClick={ () => this.props.showView('days') }>
						{ date.format( this.props.dateFormat ) } 
					</td>
					<td colSpan="1" onClick={ () => this.props.showView('days') }>
						<span className="rdtBack" onClick={ () => this.props.showView('days') }>&times;</span>
					</td>
				</tr>
			</thead>
		);
