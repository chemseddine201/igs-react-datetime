...
export default function ViewNavigation( { 
	...
	return (
		<tr>
			{(viewDate.endOf('month').isAfter(minDate)) ? (<th className="rdtPrev" onClick={ onClickPrev }>
				<span>{prevIcon || '‹'}</span>
			</th>) : null}
			<th className="rdtSwitch" colSpan={ switchColSpan } onClick={ onClickSwitch } {...switchProps}>
				{ switchContent }
			</th>
			{(viewDate.endOf('month').isBefore(maxDate)) ? (<th className="rdtNext" onClick={ onClickNext }>
				<span>{nextIcon || '›'}</span>
			</th>) : null}
		</tr>
	);
